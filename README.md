...
```bash
# 终端 1：启动导入服务
python -m uvicorn app.api.http.import_server:app --host 0.0.0.0 --port 8001

# 终端 2：启动查询服务
python -m uvicorn app.api.http.query_server:app --host 0.0.0.0 --port 8002
```
...
- **[项目架构图.md](docs/项目架构图.md)** - 包含 7 个 Mermaid 图，展示系统整体架构
- **[查询图.dot](docs/查询图.dot)** - 查询状态机的 Graphviz DOT 源文件
- **[导入图.dot](docs/导入图.dot)** - 导入状态机的 Graphviz DOT 源文件
- **[系统架构图.dot](docs/系统架构图.dot)** - 完整系统分层架构
- **[docs/README.md](docs/README.md)** - 图表使用说明
...
│   ├── import_server.py   # 导入服务 (8001)
│   └── query_server.py    # 查询服务 (8002)
├── process/               # 业务编排层
│   ├── import_/agent/     # 导入图 (LangGraph)
│   │   ├── main_graph.py
│   │   ├── state.py
│   │   └── nodes/         # 8 个节点
│   └── query/agent/       # 查询图 (LangGraph)
│       ├── main_graph.py
│       ├── state.py
│       └── nodes/         # 11 个节点
├── rag/                   # 业务服务层
